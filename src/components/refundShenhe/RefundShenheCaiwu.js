...
import {getRefundList} from './RefundAction';
...
// 客服总监
class RefundShenheCaiwu extends Component {
    ...
    getOrderList = () => {
        ...
        getRefundList(
            applytimeEnd, applytimeStr, levelId, mobile, nickname, orderTimeEnd, orderTimeStr, ordersIdStr, pageno, pagesize,
            payTimeEnd, payTimeStr, phaseId, refundnum, refundtimeEnd, refundtimeStr, status, refundType,2
        ).then(data => {
            if (data.meta.code === "0") {
                console.log(data);
            } else {
                    ...
                    <span>
                         {
                             record.refundType === 0 ?
                                 ...
                                 record.approveStatus === 1 ? // 0 待客服总监审核 1 待财务总监审核 2 驳回 3 退款成功 4 退款失败 5 已取消
                                     <Button
                                         type="primary"
                                         onClick={() => {
                                             this.setState({
                                     ...
                                     : record.approveStatus === 0 ||record.approveStatus === 2 ||record.approveStatus === 3 || record.approveStatus === 4 ||record.approveStatus === 5?
                                     <Button
                                         type="primary"
                                         onClick={() => {
                                             this.setState({
                ...
                >
                    ...
                    <div className="my-horizontal-div margin-left101">
                        <Input
                            ...
                            type={'number'}
                            onChange={e => {
                                this.setState({phoneNum: parseFloat(e.target.value)});
                            }}
                            style={{width: 150}}
                        />
                    </div>
                ...
                <Modal
                    ...
                    width={1000}>
                    {
                        this.state.showTuikuan?
                            <RefundInfo commitOver={this.commitOver} type1={2} refundId={this.state.refundId} type={this.state.refundType1}/>
                            :null
                    }
                </Modal>
            </div>
...
function mapStateToProps(state, props) {
    return {
        ...
        refundList: state.refundShenheList,
    };
}
...
export default connect(mapStateToProps, mapDispatchToProps)(RefundShenheCaiwu);
