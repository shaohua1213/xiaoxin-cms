...
// 客服总监
class RefundShenheKefu extends Component {
    ...
    getOrderList = () => {
        ...
        getRefundList(
            applytimeEnd, applytimeStr, levelId, mobile, nickname, orderTimeEnd, orderTimeStr, ordersIdStr, pageno, pagesize,
            payTimeEnd, payTimeStr, phaseId, refundnum, refundtimeEnd, refundtimeStr, status, refundType,1
        ).then(data => {
            if (data.meta.code === "0") {
                console.log(data);
            } else {
                    ...
                    <span>
                         {
                             record.refundType === 0 ?
                                 ...
                                 record.approveStatus === 0 ? // 0 待客服总监审核 1 待财务总监审核 2 驳回 3 退款成功 4 退款失败 5 已取消
                                     <Button
                                         type="primary"
                                         onClick={() => {
                                             this.setState({
                                     ...
                                     : record.approveStatus === 1 ||record.approveStatus === 2 ||record.approveStatus === 3 || record.approveStatus === 4 ||record.approveStatus === 5?
                                     <Button
                                         type="primary"
                                         onClick={() => {
                                             this.setState({
                ...
                <Modal
                    ...
                    width={1000}>
                    {
                        this.state.showTuikuan?
                            <RefundInfo commitOver={this.commitOver} type1={1} refundId={this.state.refundId} type={this.state.refundType1}/>
                            :null
                    }
                </Modal>
            </div>
...
export default connect(mapStateToProps, mapDispatchToProps)(RefundShenheKefu);
